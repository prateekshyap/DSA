...
class Trie {
    ...
    public int getSecond(String str, int depth) {
        ...
        //try to keep the bits as equal as possible and move till end
...
class Solution{
    static int minxorpair(int n, int nums[]){
        ...
        int maxDepth = 0;
        ArrayList<Integer> list = new ArrayList<Integer>(); //stores all possible combinations
        Trie trie = new Trie();
        ...
        //find out the minimum xor value possible
        int min = Integer.MAX_VALUE;
        for (Integer firstInd : list)
        {
            int second = trie.getSecond(binary[firstInd], maxDepth);
